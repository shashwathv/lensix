...
class EnhancedOverlay(QMainWindow):
    ...
    def _capture_background(self):
        """
        Multi-strategy capture chain. Tries each method in order, stops at
        first success. Falls back to plain dark overlay (GNOME 49+ Wayland).

        Strategy 1: mss  — works on X11 and KDE/any XWayland session
        Strategy 2: spectacle — KDE Plasma 6 native Wayland
        Strategy 3: grim — wlroots compositors (Sway, Hyprland)
        Strategy 4: gnome-screenshot — older GNOME pre-46
        Fallback: None — plain dark overlay, app still fully functional
        """
        tmp = config.temp_dir / "background_capture.png"

        # Strategy 1: mss (works on X11 and KDE Wayland via XWayland)
        try:
            with mss.mss() as sct:
        ...
        if which("spectacle"):
            try:
                ...
                print(f"Failed to capture with spectacle: {e}", file=sys.stderr)

        # Strategy 3: grim (wlroots — Sway, Hyprland)
        if which("grim"):
            try:
                r = subprocess.run(["grim", str(tmp)],
                                   capture_output=True, timeout=5)
                if r.returncode == 0 and tmp.exists():
                    self.screenshot_pixmap = QPixmap(str(tmp))
                    tmp.unlink(missing_ok=True)
                ...
                print(f"Failed to capture with grim: {e}", file=sys.stderr)

        # Strategy 4: gnome-screenshot (older GNOME pre-46)
        if which("gnome-screenshot"):
            try:
                r = subprocess.run(["gnome-screenshot", "-f", str(tmp)],
                                   capture_output=True, timeout=5)
                if r.returncode == 0 and tmp.exists():
                    self.screenshot_pixmap = QPixmap(str(tmp))
                    tmp.unlink(missing_ok=True)
                ...
                print(f"Failed to capture with gnome-screenshot: {e}", file=sys.stderr)

        # All strategies failed — dark overlay fallback (GNOME 49+ Wayland)
        print("⚠ Screen capture unavailable — running with dark overlay", file=sys.stderr)
        self.screenshot_pixmap = None
    ...
    def paintEvent(self, _):
        ...
        if self.screenshot_pixmap:
            ...
            # GNOME 49+ Wayland fallback — plain dark overlay
            p.fillRect(self.rect(), QColor(5, 8, 15, 230))
        ...
        # Reveal screenshot inside selection (if available)
        if self.screenshot_pixmap:
            p.save()
            p.setClipPath(clip)
            p.drawPixmap(self.rect(), self.screenshot_pixmap)
    ...
    def _save_selection(self, rect: QRectF):
        try:
            ...
            if self.screenshot_pixmap:
                ...
                # GNOME 49+ Wayland fallback — grab fresh region with mss
                with mss.mss() as sct:
                    region = {"left": x, "top": y, "width": w, "height": h}
                    simg = sct.grab(region)
                    img  = Image.frombytes("RGB", simg.size, simg.rgb)
                    img.save(str(config.screenshot_path))
